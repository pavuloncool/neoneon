      ...
      <FadeUp delay={0.25}>
        <section className="grid md:grid-cols-3 gap-px bg-border mb-20">
          ...
          <Link href="/data-visualisation" className="bg-paper dark:bg-ink dark:border dark:border-white/10 p-10 group hover:bg-ink dark:hover:bg-white transition-colors duration-500">
            <p className="text-xs tracking-widest text-muted group-hover:text-paper/60 dark:group-hover:text-ink/60 mb-3 uppercase transition-colors">Category</p>
            <h2 className="font-display text-4xl font-light group-hover:text-paper dark:text-white dark:group-hover:text-ink transition-colors">Data Visualisation</h2>
          </Link>
        </section>
      </FadeUp>
      {recent.length > 0 && (
            ...
            {recent.map((article) => (
              <StaggerItem key={article.id}>
                ...
                >
                  ...
                  <span className="text-sm text-muted group-hover:text-paper/60 dark:group-hover:text-ink/60 shrink-0 transition-colors duration-300">{formatDate(article.published_at ?? article.created_at)}</span>
                </Link>
              </StaggerItem>
            ))}
          </StaggerList>
